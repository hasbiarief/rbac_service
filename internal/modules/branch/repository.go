...
func (r *BranchRepository) GetAll(limit, offset int, search string, companyID *int64, isActive *bool) ([]*Branch, error) {
	baseQuery := `
...
func (r *BranchRepository) GetByID(id int64) (*Branch, error) {
	query := `
		SELECT id, company_id, name, code, parent_id, level, path, is_active, created_at, updated_at
		FROM branches
		WHERE id = $1
...
func (r *BranchRepository) GetByCompany(companyID int64, includeHierarchy bool) ([]*Branch, error) {
	query := `
		SELECT id, company_id, name, code, parent_id, level, path, is_active, created_at, updated_at
		FROM branches
...
func (r *BranchRepository) GetChildren(parentID int64) ([]*Branch, error) {
	query := `
		SELECT id, company_id, name, code, parent_id, level, path, is_active, created_at, updated_at
		FROM branches
